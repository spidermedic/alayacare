...
def main():
    ...
    try: # is is in place to catch any error codes
        ...
        for visitDate, new_visits in new_schedule.items():
            if visitDate in saved_schedule:
        ...
        print(f"{sys.argv[0]} exited with error code {e.returncode}")
        sendmail(f"{sys.argv[0]} exited with error code {e.returncode}")
